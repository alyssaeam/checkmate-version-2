...
if ($_SERVER['REQUEST_METHOD'] === 'POST'
    ...
    $redir_uid = $uid;
    header("Location: users.php?success=password_reset&uid=$redir_uid");
    exit();
}
...
if ($_SERVER['REQUEST_METHOD'] === 'POST'
    ...
    $pdo->prepare("UPDATE tasks SET user_id = NULL WHERE user_id = ?")
        ->execute([$uid]);
    $pdo->prepare("
        DELETE FROM tasks WHERE created_by = ? AND is_private = 1
    ")->execute([$uid]);
    ...
    $us = $pdo->prepare("
        SELECT u.*,
               (SELECT COUNT(*) FROM tasks
                WHERE user_id = u.id) AS task_count
        FROM users u
...
<div class="alert alert-success alert-dismissible fade show mb-4">
    ...
    <?php echo match($_GET['success']) {
        ...
        'password_reset' => 'Password has been reset to <strong>checkmate123</strong>. '
                          . 'The member will be prompted to change it on next login.',
        default          => 'Done.'
    }; ?>
...
<div class="row g-4">
        ...
        <div class="cm-card"
            ...
            <div style="padding:10px 14px;
                ...
                <form method="GET"
                    ...
                    <?php endif; ?>
                    <button type="submit" class="cm-icon-btn"
                            title="Search">
    ...
    <div class="col-lg-8">
        <?php if ($view_user):
            $vname = $view_user['username']
                     ?? explode('@', $view_user['email'])[0];
            ...
            <div class="cm-card-body">
                ...
                <div class="cm-notice cm-notice-info mb-4"
                    ...
                    <div class="cm-notice-body">
                        ...
                        <div>
                            ...
                            <div class="cm-notice-text">
                                ...
                                and flags the account to require a
                                password change on next login.
                                Share the temporary password with the
                                member privately.
                            </div>
                        </div>
                    </div>
                    <form method="POST" class="mt-3"
                          ...
                          ?>? Their password will be reset to checkmate123."
                ...
                <div class="cm-notice cm-notice-danger mb-4">
                    <div class="cm-notice-body">
                        ...
                        <div>
                            ...
                            <div class="cm-notice-text">
                                ...
                                unassigns their tasks and deletes
                                their private tasks. This cannot be
                                undone.
                            </div>
                        </div>
                    </div>
                    <form method="POST" class="mt-3"
                          data-confirm="Remove <?php
                              echo htmlspecialchars(addslashes($vname));
                          ?>? All their private tasks will be deleted."
                    ...
                    <div class="cm-list-item">
                        <div class="cm-list-item-body">
                            ...
                            <div class="cm-list-item-meta">
                                ...
                                <?php if (strtotime($t['due_datetime'])
                                          < time()
                                          && (int)$t['status_id'] < 3): ?>
            ...
            <form method="POST">
                <div class="modal-body p-4">
                    ...
                    <div class="alert alert-info mb-0">
                        ...
                        <small>
                            The member will be prompted to change it
                            on first login.
                        </small>
                    </div>
                </div>
