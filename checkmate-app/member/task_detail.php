...
<div class="cm-detail-layout">
    <div class="cm-card">
        ...
        <div class="cm-detail-header">
            <div class="cm-detail-header-left">
                <div class="cm-detail-badges">
                    ...
                    <span class="cm-tag cm-tag-warning">
                        ...
                        Assigned by
                        <?php echo htmlspecialchars($task['creator_name'] ?? 'Admin'); ?>
                    </span>
                    <?php endif; ?>
                    ...
                    <span class="cm-tag"
                          style="background:<?php echo htmlspecialchars($task['category_color']); ?>20;
                                 color:<?php echo htmlspecialchars($task['category_color']); ?>;
                                 border:1px solid <?php echo htmlspecialchars($task['category_color']); ?>40">
            ...
            <span class="cm-status-badge cm-status-<?php echo $status_cls; ?> cm-status-lg">
        ...
        <div class="cm-detail-section">
            <div class="cm-meta-grid">
                ...
                <div class="cm-meta-cell <?php echo $is_done ? 'cm-meta-success' : ($is_over ? 'cm-meta-danger' : ''); ?>">
                    <?php if ($is_done): ?>
                    <div class="cm-meta-label">
                        <i class="bi bi-check-circle me-1"></i>Completed On
                    </div>
                    <div class="cm-meta-value text-success">
                        <?php echo !empty($task['completed_at']) ? date('F j, Y', strtotime($task['completed_at'])) : '—'; ?>
                    </div>
                    <div class="cm-meta-sub">
                        <?php echo !empty($task['completed_at']) ? date('g:i A', strtotime($task['completed_at'])) : ''; ?>
                    </div>
                    <?php elseif ($is_over): ?>
                    ...
                    <div class="cm-meta-value text-danger">
                        <?php echo $d > 0 ? "$d day" . ($d > 1 ? 's' : '') . " ago" : "$h hour" . ($h > 1 ? 's' : '') . " ago"; ?>
                    </div>
                    <?php elseif ($is_conf): ?>
                    <div class="cm-meta-label">
                        <i class="bi bi-hourglass-split me-1"></i>Status
                    </div>
                    <div class="cm-meta-value" style="color:var(--warning)">
                        Waiting for admin
                    </div>
                    <?php else: ?>
                    <div class="cm-meta-label">
                    ...
                    $rem = $days > 0 ? "$days day" . ($days > 1 ? 's' : '') : ($hrs > 0 ? "$hrs hour" . ($hrs > 1 ? 's' : '') : "$mins min");
                ...
                <div class="cm-meta-cell">
                    ...
                    <div class="cm-meta-value">
                        <?php echo $is_own ? 'You' : htmlspecialchars($task['creator_name'] ?? 'Admin'); ?>
                    </div>
                </div>
            ...
            <div class="d-flex gap-2 flex-wrap">
                ...
                <form method="POST">
                    ...
                    <button type="submit" class="cm-btn cm-btn-success">
                        ...
                        <?php echo $is_own ? 'Mark as Done' : 'Submit for Confirmation'; ?>
                    </button>
                </form>
                <?php endif; ?>
                <?php if ($is_own && $sid === 1): ?>
                <a href="tasks.php?edit=<?php echo $task['id']; ?>&filter=<?php echo $filter; ?>"
                   class="cm-btn cm-btn-primary">
                    <i class="bi bi-pencil me-1"></i>Edit
                </a>
                <?php endif; ?>
                <?php if ($is_own): ?>
                <form method="POST" onsubmit="return confirm('Delete this task permanently?')">
