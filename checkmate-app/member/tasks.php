...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($action === 'mark_done') {
        $check = $pdo->prepare("
            SELECT id, created_by, user_id FROM tasks WHERE id=?
        ");
        ...
        header("Location: tasks.php?filter=$f&success=marked_done"); exit();
    }

    if ($action === 'delete') {
        $pdo->prepare("DELETE FROM tasks WHERE id=? AND created_by=?")
            ->execute([$task_id, $member_id]);
        header("Location: tasks.php?filter=$f&success=deleted"); exit();
    }
    ...
    if ($action === 'create') {
        ...
        if (!empty($title) && !empty($due)) {
            $pdo->prepare("
                INSERT INTO tasks
                    (title,description,due_datetime,category_id,priority,
                     is_private,created_by,status_id)
                VALUES (?,?,?,?,?,1,?,1)
            ")->execute([$title,$description,$due,
                          $category_id,$priority,$member_id]);
        }
        header("Location: tasks.php?filter=private&success=created"); exit();
    }
    ...
    if ($action === 'edit') {
        ...
        if ($check->fetch()) {
            ...
            $pdo->prepare("
                UPDATE tasks
                SET title=?,description=?,due_datetime=?,
                    category_id=?,priority=?
                WHERE id=? AND created_by=?
            ")->execute([$title,$description,$due,
                          $category_id,$priority,$task_id,$member_id]);
        }
        header("Location: tasks.php?filter=private&success=updated"); exit();
    }
}

// ── LOAD DATA (after redirects handled) ──────────────────
$filter   = $_GET['filter'] ?? 'private';
$per_page = 8;
$page     = max(1, (int)($_GET['page'] ?? 1));

$priv_cnt = $pdo->prepare("
    SELECT COUNT(*) FROM tasks WHERE created_by=? AND status_id!=3");
$priv_cnt->execute([$member_id]);
$priv_count = (int)$priv_cnt->fetchColumn();

$asgn_cnt = $pdo->prepare("
    SELECT COUNT(*) FROM tasks
    WHERE user_id=? AND created_by!=? AND status_id!=3");
$asgn_cnt->execute([$member_id,$member_id]);
$asgn_count = (int)$asgn_cnt->fetchColumn();

$done_cnt = $pdo->prepare("
    SELECT COUNT(*) FROM tasks
    WHERE (created_by=? OR user_id=?) AND status_id=3");
$done_cnt->execute([$member_id,$member_id]);
$done_count = (int)$done_cnt->fetchColumn();

if ($filter==='assigned') {
    $cp=$pdo->prepare("SELECT COUNT(*) FROM tasks
        WHERE user_id=? AND created_by!=? AND status_id!=3");
    ...
    $cp=$pdo->prepare("SELECT COUNT(*) FROM tasks
        WHERE created_by=? AND status_id!=3");
...
if ($filter==='assigned') {
    $stmt=$pdo->prepare($base."
        WHERE t.user_id=? AND t.created_by!=? AND t.status_id!=3
        ORDER BY t.due_datetime ASC LIMIT $per_page OFFSET $offset");
    $stmt->execute([$member_id,$member_id]);
} elseif ($filter==='done') {
    $stmt=$pdo->prepare($base."
        WHERE (t.created_by=? OR t.user_id=?) AND t.status_id=3
        ORDER BY t.completed_at DESC LIMIT $per_page OFFSET $offset");
    $stmt->execute([$member_id,$member_id]);
} else {
    $stmt=$pdo->prepare($base."
        WHERE t.created_by=? AND t.status_id!=3
...
$ov=$pdo->prepare("SELECT COUNT(*) FROM tasks
    WHERE (created_by=? OR user_id=?)
    AND due_datetime<NOW() AND status_id NOT IN(2,3)");
    ...
    <div>
        ...
        <div class="d-flex gap-2 mt-1 flex-wrap">
            <span class="cm-tag cm-tag-neutral">
                <?php echo $priv_count+$asgn_count; ?> Active
            </span>
...
<ul class="nav nav-pills mb-4">
    <li class="nav-item">
        <a class="nav-link <?php echo $filter==='private'?'active':''; ?>"
           href="<?php echo memberPgUrl('private',1); ?>">
            Private
    ...
    <li class="nav-item">
        <a class="nav-link <?php echo $filter==='assigned'?'active':''; ?>"
           href="<?php echo memberPgUrl('assigned',1); ?>">
            Assigned
            <span class="badge bg-light text-dark ms-1">
                <?php echo $asgn_count; ?>
            </span>
    ...
    <li class="nav-item">
        <a class="nav-link <?php echo $filter==='done'?'active':''; ?>"
           href="<?php echo memberPgUrl('done',1); ?>">
            Done
            <span class="badge bg-light text-dark ms-1">
                <?php echo $done_count; ?>
            </span>
            ...
            <div class="d-flex justify-content-between align-items-start gap-3">
                <div class="flex-grow-1 min-width-0">
                    ...
                    <p style="font-size:12px;color:var(--text-muted);margin:0 0 4px">
                        <?php echo htmlspecialchars(
                            substr($task['description'],0,80)); ?>…
                    </p>
                    <?php endif; ?>
                    <div style="font-size:11px;color:<?php
                        echo $is_over?'var(--danger)':'var(--text-muted)'; ?>">
                ...
                <div class="d-flex flex-column align-items-end gap-2"
                    ...
                    <span class="cm-status-badge cm-status-<?php
                        echo $sid===3?'done':($sid===2?'confirm':'pending'); ?>">
                    ...
                    <div class="d-flex gap-1 flex-wrap justify-content-end">
                        ...
                        <a href="task_detail.php?id=<?php echo $task['id'];
                            ?>&from=<?php echo htmlspecialchars($filter); ?>"
...
<nav class="mt-4">
    <ul class="pagination justify-content-center">
        <li class="page-item <?php echo $page<=1?'disabled':''; ?>">
            <a class="page-link"
               href="<?php echo memberPgUrl($filter,$page-1); ?>">
                &laquo;
            </a>
        </li>
        ...
        <li class="page-item <?php echo $i===$page?'active':''; ?>">
            <a class="page-link"
               href="<?php echo memberPgUrl($filter,$i); ?>">
                <?php echo $i; ?>
            </a>
        </li>
        <?php endfor; ?>
    ...
    <div class="modal-dialog modal-lg">
        <div class="modal-content">
            ...
            <form method="POST">
                ...
                <input type="hidden" name="task_id"
                       value="<?php echo (int)$edit_task['id']; ?>">
                <div class="modal-body p-4">
                    ...
                    <div class="mb-3">
                        ...
                        <textarea name="description"
                            ...
                            echo htmlspecialchars($edit_task['description']??'');
                        ?></textarea>
                    </div>
                        ...
                        <div class="col-md-6">
                            ...
                            <input type="datetime-local" name="due_datetime"
                                   ...
                                   value="<?php echo date('Y-m-d\TH:i',
                                       strtotime($edit_task['due_datetime'])); ?>">
                        </div>
                        <div class="col-md-6">
                            ...
                            <select name="category_id" class="form-select">
                                ...
                                <option value="<?php echo $cat['id']; ?>"
                                        style="color:<?php echo htmlspecialchars(
                                            $cat['color']); ?>"
                                        <?php echo (int)$edit_task['category_id']
                                            ===(int)$cat['id']?'selected':''; ?>>
                    ...
                    <div class="col-md-6">
                        ...
                        <select name="priority" class="form-select">
                            ...
                            <option value="<?php echo $i; ?>"
                                    <?php echo (int)$edit_task['priority']===$i
                                        ?'selected':''; ?>>
                                <?php echo $i.' '.str_repeat('⭐',$i); ?>
                            </option>
                            <?php endfor; ?>
                        </select>
                    </div>
